# coding=utf-8
"""
dnawalker.transformer.train — Transformer 主训练脚本
dnawalker.transformer.train — Transformer main training script

用法 / Usage:
    python -m dnawalker.transformer.train
    python -m dnawalker.transformer.train --smoke
"""
...
from .data import load_and_preprocess_data_3d
...
def train(smoke_test: bool = False, parent_config_override=None, transformer_config_override=None):
    ...
    # Writers own directory creation; configuration getters are side-effect free.
    for artifact_path in (MODEL_SAVE, Y_SCALER_PATH):
    ...
    # 与 dataset.py 的划分口径一致，种子取自 parent_config。
    seed = parent_config.get_random_seed()
    seed_everything(seed)
    print(f"  随机种子      : {seed} (固定随机源 / seeded run)")
    ...
    if train_loader is None:
        raise RuntimeError(f"Training data could not be loaded: {DATASET_PATH}")
    dataset_sha256 = sha256_file(DATASET_PATH)
    y_scaler_sha256 = sha256_file(Y_SCALER_PATH)
    split_metadata = {}
    split_manifest = parent_config.get_split_manifest_file()
    if split_manifest is not None:
        split_metadata = {
            "split_manifest_sha256": sha256_file(split_manifest),
            "train_subset_size": int(parent_config.get_train_subset_size()),
        }

    # ── 2. 初始化模型 ────────────────────────────────────────────────────────
    # 复用共享的设备选择 (CUDA>MPS>CPU)，与推理适配器 (dnawalker.transformer.inference) 口径一致。
    device = pick_device()
    ...
    # ── 3. 损失函数 / 优化器 / 调度器 ────────────────────────────────────────
    # 损失函数：由 [TRAINING] loss_weight_mode 控制 (与 CNN 训练器口径一致)。
    #   'adaptive' (默认): 逐参数权重按验证集 MSE EMA 自适应 (历史行为)。
    #   'none'          : 标准等权 MSE (param_weights 恒为 1，下方自适应更新被跳过)。
    loss_weight_mode = parent_config.get_loss_weight_mode()
    ...
    if adaptive_weighting:
        print(f"  损失函数: 自适应加权 MSE Loss (loss_weight_mode={loss_weight_mode})")
    else:
        print("  损失函数: 标准等权 MSE Loss (loss_weight_mode=none)")
    ...
    for epoch in range(1, NUM_EPOCHS + 1):
        ...
        # --- 自适应损失加权 (仅 loss_weight_mode != none 时启用) ---
        ...
        if avg_val_mse < best_val_mse:
            ...
            torch.save({
                ...
                'model_seed' : int(parent_config.get_random_seed()),
                'split_seed' : int(parent_config.get_split_seed()),
    ...
    if not checkpoint_saved:
        raise RuntimeError(
            ...
            f"reuse a stale model at {MODEL_SAVE}"
        )
...
def main(argv=None):
    """Run the Transformer training CLI."""
    parser = argparse.ArgumentParser(
        description="DNA Walker Transformer Trainer"
    )
    parser.add_argument(
        "--config",
    ...
    parser.add_argument(
        "--smoke",
        action="store_true",
        help="Run two epochs to validate the training pipeline.",
    )
