# coding=utf-8
"""Transformer held-out and experimental-curve evaluation workflows."""
...
RESULTS_DIR = os.fspath(
    ROOT_RESULTS_DIR / "evaluation" / "transformer"
)
...
def evaluate_testset(
    ...
    """Evaluate direct Transformer predictions on the fixed test split."""
...
def eval_one(predictor, pc, ranges, data_path, ensemble, noise_std,
    ...
    return evaluate_refined_dataset(
        ...
        seed=seed,
        refine_on=refine_on,
        load_model_input_fn=lambda: load_and_smooth_experimental_curves(
            pc,
            data_path,
            verbose=False,
            on_load_error="raise",
        ),
        load_interpolated_curves_fn=lambda: (
            raw_experiment_curves_on_sim_axis(data_path)
        ),
...
def plot_dual(results, out_path):
    ...
    fig.suptitle(
        'Transformer + physics refinement: original vs. generalization',
...
def evaluate_experimental(args):
    ...
    require_current = (
        ...
        or bool(args.config or args.transformer_config or args.model)
    )
    provenance = (
        require_current_artifact_provenance(predictor, pc)
        if require_current
        else evaluation_provenance(predictor, pc)
    )
    name_map = build_name_map(pc.get_trainable_param_names())
    ...
    print("\n================ Dual-dataset summary (Transformer) ================")
...
def main(argv=None):
    parser = argparse.ArgumentParser(
        description="Evaluate the Transformer model."
    )
    ...
    testset.add_argument("--transformer-config")
    ...
    testset.add_argument("--tag", default="transformer")
    ...
    experimental.add_argument("--config")
    experimental.add_argument("--transformer-config")
    experimental.add_argument("--model")
    ...
    experimental.add_argument("--tag", default="exp")
